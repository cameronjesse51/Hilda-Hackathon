-- Replace fuzzy ILIKE program matching with exact CIP-code lookup and add
-- credential-level filtering.  When the Python layer resolves a program name
-- to CIP codes via the study_areas_by_code catalog, the RPC matches by code;
-- otherwise it falls back to the original ILIKE text path.

-- Performance index for the post-search enrichment query.
create index if not exists idx_institution_specialties_unitid_cipcode
    on public.institution_specialties ("UNITID", "CIPCODE");

-- Drop every existing overload of search_colleges.
do $$
declare
...
create function public.search_colleges(
  ...
  requested_cip_codes text[] default null,
  filter_credential text default null
)
returns table (
  unitid text,
  name text,
...
as $$
  ...
  left join lateral (
    select
      ...
      array_agg(distinct specialty_row."CREDDESC")
        filter (where specialty_row."CREDDESC" is not null) as credential_levels,
      sum(coalesce(specialty_row."AWARDS_LAST_YEAR", 0)) as awards_last_year
    from public.institution_specialties as specialty_row
    where specialty_row."UNITID" = college.unitid
      and (
        -- Prefer exact CIP code match when codes are resolved
        (requested_cip_codes is not null
         and specialty_row."CIPCODE"::text = any(requested_cip_codes))
        or
        -- Fall back to text match when no CIP codes resolved
        (requested_cip_codes is null
         and requested_program is not null
         and specialty_row."CIPDESC" ilike '%' || requested_program || '%')
      )
      and (filter_credential is null
           or specialty_row."CREDDESC" = filter_credential)
  ...
  where college.embedding is not null
    ...
    and (
      (requested_cip_codes is null and requested_program is null)
      or specialty.programs is not null
    )
